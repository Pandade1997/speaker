...
from data.sparse_image_warp import sparse_image_warp
...
def add_delta(utt, delta_order):
    ...
    for i in range(1, delta_order + 1):
        max_offset = int((len(scales[i]) - 1) / 2)
...
class BaseDataset(Dataset):
    def __init__(self, opt, data_dir):
        ...
        self.agument_feat = opt.agument_feat
        self.augment_w_para = opt.augment_w_para
        self.augment_f_para = opt.augment_f_para
        self.augment_f_num = opt.augment_f_num
        self.augment_t_para = opt.augment_t_para
        self.augment_t_num = opt.augment_t_num
        self.augment_p_para = opt.augment_p_para
    ...
    def parse_feat(self, feat_path, delta_order=0, vad_idx=None, cmvn=None, left_context_width=0,
    ...
    def spect_agument_feat(self, in_feat):
        if in_feat is None:
            return None
        time_steps = in_feat.shape[0]
        raw_feat_size = in_feat.shape[1]
        if self.cmvn is not None:
            raw_mean = -self.cmvn[0, :raw_feat_size]
        else:
            raw_mean = np.zeros(raw_feat_size, dtype='float32')
        ...
        for _ in range(self.augment_t_num):
            ...
            t_mask_start = random.randint(0, time_steps - t_mask_len)
            in_feat[t_mask_start:t_mask_start + t_mask_len, :] = raw_mean[:]
        return in_feat

    def time_warp(self, spec, W=5):
        spec = torch.FloatTensor(spec).unsqueeze(0).transpose(1, 2)
        ...
        return warped_spectro.squeeze(3).transpose(1, 2).squeeze(0).numpy()
...
class DeepSpeakerSeqDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type != 'train':
            ...
            utter_spk_ids_batch = np.concatenate(utter_spk_ids_batch, axis=0)
            return torch.FloatTensor(utter_batch), torch.LongTensor(utter_segment_num_batch), torch.LongTensor(
                utter_spk_ids_batch)
